...
oFlamedragongrass = InheritO(oPeashooter, {
    EName: "oFlamedragongrass",
    CName: "火龙草",
    width: 73,
    height: 80,
    beAttackedPointR: 53,
    SunNum: 350,
    PicArr: ["images/Card/Plants/Flamedragongrass.png", "images/Plants/Flamedragongrass/0.gif", "images/Plants/Flamedragongrass/Flamedragongrass.gif", "images/Plants/Torchwood/SputteringFire.gif", "images/Plants/PeaBulletHit.gif"],
    Attack: 1000,
    AudioArr: ["splat1", "splat2", "splat3", "plastichit", "shieldhit", "shieldhit2"],
    Tooltip: "一次射出三行的火焰",
    Produce: '火龙草可以在三条线上同时射出火焰。<p>伤害：<font color="#FF0000">普通(火焰)</font><br>范围：<font color="#FF0000">三线</font></p>老子他妈要热死了！！！',
    getTriggerR: function(a) {
        
        return [a > 2 ? a - 1 : 1, a < oS.R ? Number(a) + 1 : a]
    },
    getTriggerRange: function(a, b, c) {
        return [[b, Math.min(c + 230, oS.W), 0]]
    },
    PrivateBirth: function(f) {
        var e = f.AttackedLX,
            d = e - 40,
        ...
        for (b in c) {
            ...
            f.BulletEle.push(NewImg(0, "images/Plants/Torchwood/SputteringFire.gif", "width:264px;height:88px;left:" + (d+88) + "px;top:" + (GetY(b) - 50) + "px;visibility:hidden;z-index:" + (3 * b + 2)))
        }
    },
    PrivateDie: function(a) {
        a.BulletEle.length = 0
    ...
    NormalAttack: function() {
        ...
        for (a in c.oTrigger) {
            ...
            oSym.addTask(1, function(h, l, j, e, p, k, o, m, q, i) {
                var n, g = GetC(p),
                    f = oZ["getZ" + e](p, k);
                o == 0 && i[k + "_" + g] && m != g && (PlayAudio("jalapeno"), o = 1, j = 40, m = g, l.src = "images/Plants/PB" + o + e + ".gif");
                oSym.addTask(50, ClearChild, [l]);
                f && f.Altitude == 1 ? (f[{
                    "-1": "getSnowPea",
                    0: "getPea",
                    1: "getFirePea"
                    ...
                    ) : (p += (n = !e ? 5 : -5)) < oS.W && p > 100 ? (oSym.addTask(1, arguments.callee, [h, l, j, e, p, k, o, m, q, i])) : ClearChild(l)
            }, [d, $(d), 40, 0, c.AttackedLX, a, 0, 0, c.AttackedLX - 40, oGd.$Torch])
        }
    }
}),
...
oYanghelin = InheritO(CPlants, {
    EName: "oYanghelin", //植物英文名称
    CName: "杨和零", //植物中文名称
    HP: 0, //血量
    coolTime: 40, //冷却时间
    width: 71, //宽度
    height: 71, //高度
    beAttackedPointR: 51, //被攻击时的右边界位置
    SunNum: -250, //生成所需阳光数量
    BKind: 0, //子弹类型
    AudioArr: ["splat1", "splat2", "splat3", "plastichit", "shieldhit", "shieldhit2"], //音频数组
    PicArr: ["images/Card/Plants/YangHeLin.png", "images/Plants/Peashooter/0.gif", "images/Plants/Peashooter/Peashooter.gif", "images/Plants/PB00.gif", "images/Plants/PeaBulletHit.gif"], //图片路径数组
    Tooltip: "阳光补给", //提示信息
    Produce: '纯属虚构 如有雷同 纯属巧合', //生成描述
    //创建时的私有函数
    PrivateBirth: function(a) {
        a.BulletEle = NewImg(0, a.PicArr[3], "left:" + (a.AttackedLX - 40) + "px;top:" + (a.pixelTop + 3) + "px;visibility:hidden;z-index:" + (a.zIndex + 2));
        var c = this;
...
oYanghelin2 = InheritO(CPlants, {
    ...
    beAttackedPointR: 53,
    SunNum: 300,
    PicArr: ["images/Card/Plants/YangHeLin.png", "images/Plants/SunFlower/0.gif", "images/Plants/SunFlower/SunFlower1.gif", "images/Plants/SunFlower/SunFlower.gif"],
    Tooltip: "提供你额外的阳光(死亡后游戏结束)",
    Produce: '纯属虚构 如有雷同 纯属巧合',
    PrivateDie: function(a) {
        a.BulletEle = null;
        AppearSun(0, 0, 0, 0);
        GameOver();
    ...
    PrivateBirth: function(a) {
        oS.ProduceSun ? oSym.addTask(50,
        function(d, c, b) {
            $P[d] && (a.ChangePosition($(d), 1), oSym.addTask(50,
            function(h, g, f, e) {
                $P[h] && (AppearSun(Math.floor(g + Math.random() * 41), f, 10, 0), oSym.addTask(100,
                function(i) {
                    $P[i] && a.ChangePosition($(i), 0)
                },
                [h]), oSym.addTask(50, e, [h, g, f]))
            },
            [d, c, b, arguments.callee]))
        },
        [a.id, GetX(a.C) - 40, GetY(a.R)]) : a.getHurt = function(f, c, b) {
            ...
            switch (c) {
            case 0:
                var d = (e.HP -= b); ! (d % 100) && (AppearSun(Math.floor(GetX(e.C) - 40 + Math.random() * 41), GetY(e.R), 10, 0), oSym.addTask(50,
                function(h, g) {
                    AppearSun(Math.floor(GetX(h) - 40 + Math.random() * 41), GetY(g), 10, 0)
                },
                [e.C, e.R]), d < 1 ? e.Die() : oSym.addTask(50,
                function(h, g) {
                    AppearSun(Math.floor(GetX(h) - 40 + Math.random() * 41), GetY(g), 10, 0)
...
oBlover = InheritO(CPlants, {
	...
	Dispel:function(){ //吹风
		...
		oSym.addTask(150,function(id){
			var p=$P[id];
			p && p.Die();
			oSym.addTask(2400, oGd.MoveFogLeft, []); //24s后恢复
		},[id]);
	}
});