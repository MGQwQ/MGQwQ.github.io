...
$group=''.$Data['group_id'].' '.$Data['sender']['nickname'].':'.$Data['message'].'<br>';
$user=''.$Data['user_id'].' '.$Data['sender']['nickname'].':'.$Data['message'].'<br>';
$all=''.$Data['group_id'].' '.$Data['sender']['nickname'].':'.$Data['message'].'<br>';
//判断类型并存入对应日志目录
if(!empty($Data['group_id'])){
 file_put_contents('./group.txt',$group.PHP_EOL, FILE_APPEND);
	file_put_contents('./Data/Log/Group/'.date('Y-m-d',time()).'/'.$Data['group_id'].'.txt',$content, FILE_APPEND);
}elseif(!empty($Data['user_id'])){
 file_put_contents('./user.txt',$user.PHP_EOL, FILE_APPEND);
	...
	//排除心跳事件
	file_put_contents('./other.txt',$content, FILE_APPEND);
	file_put_contents('./Data/Log/Other/'.date('Y-m-d',time()).'.txt',$content, FILE_APPEND);
}
file_put_contents('./awa.txt',$all.PHP_EOL, FILE_APPEND);
//配置文件
